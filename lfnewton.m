        ...
        if kb(n) == 2
            ...
            if Qmax(n) ~= 0
                ...
                if iter <= 7  % Between the 2th & 6th iterations
                    if iter > 2  % the Mvar of generator buses are tested.
                        if Qgc < Qmin(n)
                            Vm(n) = Vm(n) + 0.01; % If not within limits Vm(n) is changed in steps of 0.01 pu to bring the generator Mvar within the specified limits.
                        elseif Qgc > Qmax(n)
                            Vm(n) = Vm(n) - 0.01;
                        end
                    else
